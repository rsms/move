...
res = move.compile('x = foo._move_slice(1, 2)', compilerOptions);
assert.notEqual(res.indexOf('x = foo._move_slice(1, 2)'), -1);

res = move.compile('x = foo[1:2]', compilerOptions);
assert.notEqual(res.indexOf('x = foo._move_slice(1, 2)'), -1);

res = move.compile('x = foo[1:]', compilerOptions);
assert.notEqual(res.indexOf('x = foo._move_slice(1)'), -1);

res = move.compile('x = foo[:2]', compilerOptions);
assert.notEqual(res.indexOf('x = foo._move_slice(0, 2)'), -1);

res = move.compile('x = foo[:]', compilerOptions);
assert.notEqual(res.indexOf('x = foo._move_slice(0)'), -1);
